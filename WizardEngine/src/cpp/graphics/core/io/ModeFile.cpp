...
namespace engine {
    ...
    uint32_t ModeFile::vertexCounter = 0;
    uint32_t ModeFile::normalsCounter = 0;
    uint32_t ModeFile::uvsCounter = 0;
    uint32_t ModeFile::indexCounter = 0;
    std::vector<ObjVertex> ModeFile::vertices = {};
    std::vector<glm::vec2> ModeFile::uvs = {};
    std::vector<glm::vec3> ModeFile::normals = {};
    std::vector<uint32_t> ModeFile::indices = {};
    std::vector<Face> ModeFile::faces = {};
    std::vector<ObjMesh> ModeFile::meshes = {};
    std::string_view ModeFile::_fileName;

    BaseMeshComponent<ObjVertex> ModeFile::read(const std::string &fileName) {
        _fileName = fileName;
    ...
    void ModeFile::createMesh() {
        if (uvs.empty()) {
            ENGINE_WARN("ModeFile: No UVs for {0} file!", _fileName);
        }

        if (normals.empty()) {
            ENGINE_WARN("ModeFile: No normals for {0} file!", _fileName);
        }
    ...
    void ModeFile::clearVertexCache() {
        vertices.clear();
        uvs.clear();
        normals.clear();
        indices.clear();
    ...
    void ModeFile::clearMeshCache() {
        meshes.clear();
    }

    void ModeFile::resetVertexCounters() {
        vertexCounter = 0;
        uvsCounter = 0;
        normalsCounter = 0;
        indexCounter = 0;
