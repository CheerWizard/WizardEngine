...
namespace engine {
    ...
    void GLFrameBuffer::attachColorSpecs() {
    ...
    void GLFrameBuffer::attachDepthSpecs() {
    ...
    void GLFrameBuffer::createDrawBuffers() {
    ...
    uint32_t GLFrameBuffer::convertTextureFormat(const FramebufferTextureFormat &format) {
        switch (format) {
    ...
    void GLFrameBuffer::attachColorTextures() {
    ...
    uint32_t GLFrameBuffer::getTextureTarget() {
        return specification.samples > 1 ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D;
    }

    void GLFrameBuffer::createTextures(const uint32_t &count, uint32_t *outAttachmentId) {
        glCreateTextures(getTextureTarget(), (GLsizei) count, outAttachmentId);
    }

    void GLFrameBuffer::bindTexture(const uint32_t &attachmentId) {
        glBindTexture(getTextureTarget(), attachmentId);
    }

    void GLFrameBuffer::convertTextureFormat(const uint32_t &specIndex, uint32_t &outInternalFormat, uint32_t &outFormat) {
    ...
    void GLFrameBuffer::resize(uint32_t width, uint32_t height) {
        if (width == 0 || height == 0 || width > capacity || height > capacity) {
    ...
    void GLFrameBuffer::removeAttachment(uint32_t attachmentIndex, int value) {
